function [x,output] = SR1d_uncon_L1capped(N,fc,b,pm)
%min_x .5||S*F*x-b||^2 + lambda T_\alpha(x)
...
%% parameters
lambda = 1e-5; detla = 10*lambda; alpha = 0.05;
maxit = 5*N; maxoit = 10; 
...
for oit = 1:maxoit
    ...
    for it = 1:maxit
     ...
     output.obj(oit) = sum(min(abs(x),1));
